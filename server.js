...
async function setupDB() {
  ...
  try {
    ...
    // DB tem dados — comparar versões
    const dbData   = r.rows[0].payload
    const dbVer    = parseInt(dbData._version  || '0')
    const seedVer  = parseInt(seed._version    || '0')

    // Seed só vence se versão for MAIOR e NÃO tiver forceUpdate
    // (forceUpdate foi removido — nunca mais vai sobrescrever)
    if (seedVer > dbVer) {
      await pool.query(
        "UPDATE psicoteam_data SET payload=$1, updated_at=NOW() WHERE id='main'",
        [JSON.stringify(seed)]
      )
      console.log(`✅ DB atualizado: seed v${seedVer} > db v${dbVer}`)
    } else {
      console.log(`✅ DB preservado: db v${dbVer} >= seed v${seedVer} — ${dbData.agendas?.length||0} agendas, ${dbData.pacientes?.length||0} pacientes`)
    }
  } catch(e) {
...
async function writeData(obj) {
  // Quando usuário salva dados, NUNCA reduz a versão
  const current = await readData()
  const curVer = parseInt(current._version || '0')
  if (!obj._version || parseInt(obj._version) < curVer) {
    obj._version = curVer  // mantém versão atual
  }
  if (pool) {
    try {
      await pool.query(
...
http.createServer(async (req, res) => {
  ...
  if (req.method==='POST' && url==='/api/data') {
    try {
      ...
      // Proteção: nunca apaga agendas existentes
      const current = await readData()
  ...
  setupDB().then(async () => {
    ...
    console.log(`✅ PsicoTEAM porta ${PORT} | ${data.agendas?.length||0} agendas | ${data.pacientes?.length||0} pacientes | ${slots} sessões | v${data._version||0}`)
  })
})
